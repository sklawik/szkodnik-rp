...
public OnGameModeInit()
{
	...
	mysql_query("SELECT * FROM textures");
	mysql_store_result();
...
stock LoadObjects()
{
	...
	mysql_query("SELECT uid, model, X, Y, Z, rX, rY, rZ, VW FROM objects");
...
stock LoadDoors()
{
	...
	while(mysql_fetch_row(data))
	{
		...
		if(!DoorCache[i][dDestroyed])
		{
			...
			{	
				dPickupID[DoorCache[i][dUID]] = CreateDynamicPickup(1239, 2, DoorCache[i][dOutX],DoorCache[i][dOutY], DoorCache[i][dOutZ], DoorCache[i][dOutVW], 0, -1);
				dPickupID2[DoorCache[i][dUID]] = CreateDynamicPickup(1239, 2, DoorCache[i][dInsX],DoorCache[i][dInsY], DoorCache[i][dInsZ], DoorCache[i][dInsVW], 0, -1);
			}	
			else if(DoorCache[i][dGroupUID] != 0)
			{
				dPickupID[DoorCache[i][dUID]] = CreateDynamicPickup(GetPickupModelFromGroupType(GroupCache[DoorCache[i][dGroupUID]][gType]), 2, DoorCache[i][dOutX],DoorCache[i][dOutY], DoorCache[i][dOutZ], DoorCache[i][dOutVW], 0, -1);
				if(GetMapIcon(GroupCache[DoorCache[i][dGroupUID]][gType]))
				CreateDynamicMapIcon(DoorCache[i][dOutX],DoorCache[i][dOutY], DoorCache[i][dOutZ], GetMapIcon(GroupCache[DoorCache[i][dGroupUID]][gType]), -1, -1, -1, -1, STREAMER_MAP_ICON_SD, MAPICON_LOCAL);
			}
			else if(DoorCache[i][dGroupUID] == 0 && DoorCache[i][dType] == 1)
			{
				dPickupID[DoorCache[i][dUID]] = CreateDynamicPickup(1239, 2, DoorCache[i][dOutX],DoorCache[i][dOutY], DoorCache[i][dOutZ], DoorCache[i][dOutVW], 0, -1);
			}
			else if(DoorCache[i][dType] == 0)
			{
...
stock IsDoorInAnyDoor(dooruid)
{
	...
	{
		if(DoorCache[dooruid][dOutVW]==DoorCache[i][dInsVW]  && !DoorCache[i][dDestroyed])
		{
			return 1;
		}
	}
...
{
	new query[1024];
	format(query, sizeof(query), "INSERT INTO `zones` (`minX`, `minY`, `maxX`, `maxY`) VALUES ('%f', '%f', '%f', '%f')",
	ZoneData[zoneid][zMinX],ZoneData[zoneid][zMinY], ZoneData[zoneid][zMaxX],  ZoneData[zoneid][zMaxY]);
	mysql_query(query);
}
...
{
	new query[256];
	format(query, sizeof(query), "SELECT * FROM zones WHERE id = '%d'", zoneid);
	mysql_query(query);

	new data[525];
	if(mysql_fetch_row(data))
	{
		sscanf(data, "p<|>ffffdddds[64]",
		ZoneData[zoneid][zMinX], ZoneData[zoneid][zMinY],
		ZoneData[zoneid][zMaxX], ZoneData[zoneid][zMaxY],
		ZoneData[zoneid][zID],
		ZoneData[zoneid][zYard],
...
stock LoadZones()
{	
	print(">> Rozpoczynam ladowanie stref..");
...
stock GoUp(i)
{
	...
	new query[128];
	format(query, sizeof(query), "SELECT * FROM zones WHERE id = '%d'", i);
	mysql_query(query);
...
stock ShowDialogMemberPayDay(playerid)
{
	...
	return ShowPlayerDialog(playerid, D_MEMBER_PAYDAY, DIALOG_STYLE_INPUT, header, ""HEX_WHITE"Wpisz poniżej nową wypłate dla członka:", "Gotowe", "Anuluj");
}
...
stock CreateTextDraws()
{
	...
	new str[128];
	format(str, sizeof(str), "~w~Szkodnik~b~~h~~h~-~w~RP: %02d.%02d.%02d", gyear-2000, gmonth, gday);
...
stock LoadActors()
{
	new query[64] = "SELECT * FROM actors";
	mysql_query(query);
	...
	while(mysql_fetch_row(data))
	{
		sscanf(data, "p<|>dds[64]s[32]s[32]ffffddbs[256]",
		ActorCache[i][aUID],
		ActorCache[i][aType],
		ActorCache[i][aName],
		ActorCache[i][aAnimLib],
		ActorCache[i][aAnimName],
		ActorCache[i][aPosX],
		ActorCache[i][aPosY],
		ActorCache[i][aPosZ],
		ActorCache[i][aAng],
...
stock ShowDialogMemberPerms(playerid)
{
	new guid = pVal[playerid];
	new muid = pVal2[playerid];
	new header[64], info[256]; format(header, sizeof(header), "Edytuj uprawnienia członka %s\n", PlayerCache[muid][pName]);
...
public Bus(playerid, ouid)
{
	...
	else if(keys & KEY_SECONDARY_ATTACK)
	{
		...
		new msg[128]; format(msg, sizeof(msg), "* %s wsiada do busu nr. %d kierującego się w stronę %s. *", RPName(playerid), random(100)+4000, DeleteSomething(ObjectCache[pVal[playerid]][oName]));
...
stock GetActionName(action)
{
	...
	switch(action)
	{
		case 1: name = "Chce Ciebie pocałować";
		case 2: name = "Chce Ciebie przytulić";
	...
	switch(dialogid)
	{
		...
		case D_NEARBY_PLAYERS:
		{
			if(response)
			{
				...
				new info[128]; format(info, sizeof(info), "Gracz chce oddać Ci przedmiot %s za darmo.\nCzy akceptujesz jego ofertę?\n\nWybierz Tak lub Nie.",ItemCache[iuid][iName]);
		...
		case D_CLOTCHES:
		{
			if(response)
			{
				...
				return SendClientMessage(playerid, COLOR_GRAY, "Nie znajdujesz się w sklepie odzieżowym.");
		...
		case D_ACCEPT_GROUP_ITEM:
		{
			...
			if(response)
			{
				...
				return ShowDialogInfo(playerid, "Nie pomieścisz tylu przedmiotów w ekwipunku.");
		...
		case D_GROUP_AMOUNT_ITEM:
		{
			if(response)
			{
				...
				return SendClientMessage(playerid, COLOR_GRAY, "Ten produkt w magazynie skończył się.");

				if(amount == 0)
				{
					new info[128];
					format(info, sizeof(info), "Poniżej wpisz ilość produktu jaką chcesz podać %s.", RPName(targetid));
					return ShowPlayerDialog(playerid, D_GROUP_AMOUNT_ITEM, DIALOG_STYLE_INPUT, "Podaj ilość", info, "Wyślij", "Anuluj");
				}

				if(ItemCache[iuid][iVW] < amount)
				return SendClientMessage(playerid, COLOR_GRAY, "Poprzednio podano ilość produktów do sprzedania temu graczowi, ale aż tylu nie ma w magazynie grupy.");
				...
				format(info, sizeof(info), ""HEX_YELLOW"Zakup przedmiotu %s (x%d) za $%d\n\nCzy akceptujesz  tę ofertę??", ItemCache[iuid][iName], amount, ItemCache[iuid][iCost]*amount);
		...
		case D_MAGAZINE:
		{
			if(response)
			{
				...
				for(new i; i<MAX_ITEMS; i++)
				{
					...
					{
						if(ItemCache[i][iOwner] == PlayerCache[playerid][pUID])
						return SendClientMessage(playerid, COLOR_GRAY, "Przewozisz już jakąś paczkę.");
					}
				}
				...
				{
					format(msg, sizeof(msg), "%s już dostarcza tę paczkę.",  RPName(IsUserConnected(PlayerCache[ItemCache[itemuid][iOwner]][pUID])));
		...
		case D_INTERRACT_PLAYER:
		{
			if(response)
			{
				...
				new targetid = IsUserConnected(targetuid);
				if(targetid == -1)
				return SendClientMessage(playerid, COLOR_GRAY, "Gracz opuścił grę lub wylogował się.");
				...
				if(option == 4)
				{
					...
					if(IsAnyMedicOnline)
					return ShowDialogInfo(playerid, "Możesz ściągnąć BW temu graczowi tylko wtedy, kiedy żaden medyk nie jest na służbie.");
					...
					new msg[128]; format(msg, sizeof(msg), "%s pomógł Tobie odzyskać przytomność.\nOdegraj krótką lub dłuższą akcję RolePlay z tym graczem.", RPName(playerid));
		...
		case D_APPS:
		{
			if(response)
			{
				...
				if(callerid != -1)
				{
					...
					format(msg, sizeof(msg), "> %s odebrała Twoje zgłoszenie. Na mapie zaznaczono miejsce z którego do Ciebie jedzie.", RPName(playerid));
					else
					format(msg, sizeof(msg), "> %s odebrał Twoje zgłoszenie. Na mapie zaznaczono miejsce z którego do Ciebie jedzie.", RPName(playerid));
		...
		case D_SIM:
		{
			...
			return SendPlayerMe(playerid, "wkłada kartę SIM do telefonu.");
		}
		case D_OBJECTGROUP:
		{
			if(response)
			{
				...
				return SendClientMessage(playerid, COLOR_GRAY, "Edytowany obiekt nie jest bramą.");
		...
		/*case D_MAKEBUS:
		{
			...
			{
				if(Isnull(inputtext))
				return SendClientMessage(playerid, COLOR_GRAY, "Nie wpisano nazwy przystanku. Usuń go i wprowadź nazwę jeszcze raz.");
			...
			if(response)
			{
				...
				for(new i; i<LastvUID; i++)
				{
					if(VehicleCache[i][vState] == VEHICLE_STATE_GROUP)
					{
						...
						return ShowDialogInfo(playerid, "Grupa osiągnęła limit podpisanych pod nią pojazdów.");
					}
				}
				VehicleCache[vuid][vState] = 2;
				VehicleCache[vuid][vOwner] = guid;
				new str[128]; format(str, sizeof(str), "Pojazd należy teraz do grupy: %s (UID: %d)\nJeśli jednak zmienisz zdanie, możesz go swobodnie odpisać.", GroupCache[guid][gName], guid);
			...
			if(response)
			{
				...
				switch(option)
				{
					case 1:
					{
						...
						{
							SendClientMessage(playerid, COLOR_GRAY, "Ten gracz nie należy już do tej grupy.");
						}
						return ShowDialogGroupMembers(playerid);
					}
		...
		case D_ALLGROUPS:
		{
			if(response)
			{
				...
				{
					pPage[playerid] = 0;
					return ShowDialogInfo(playerid, "Żadna grupa nie została stworzona lub lista skończyła się.");
				}
				return 1;
			}
			else
		...
		case D_BUY_REGISTER:
		{
			if(response)
			{
				if(PlayerCache[playerid][pCash] >= 300)
				{
					...
					CreateItem(playerid, 14, 0, 0, 0,0,0, "Rejestracja pojazdu");
			...
			if(response)
			{
				switch(listitem)
				{
					case 0:
					{
						...
						strreplace(PlayerCache[uid][pName], '_', ' '), (PlayerCache[uid][pGender]) ? ("Kobieta")  : ("Mężczyzna"), PlayerCache[uid][pBornDate]);
			...
			if(response)
			{
				switch(option)
				{
					...
					case 5:
					{
						...
						return SendClientMessage(playerid, COLOR_GRAY, "Nie możesz handlować pojazdem należącym do czyjejś grupy.");
						pVal[playerid] = vuid;
		...
		case D_ADMIN_DOOR:
		{
			if(response)
			{
				...
				switch(option)
				{
					...
					case 2:
					{
						return ShowPlayerDialog(playerid, D_DOOR_VW, DIALOG_STYLE_INPUT, "Zmień Virtual World drzwi", "Wpisz poniżej wartość:\n", "Zmień", "Anuluj");
					}
		...
		case D_ADOOR_CHANGE_NAME:
		{
			...
			{
				if(Isnull(inputtext))
				return ShowPlayerDialog(playerid, D_ADOOR_CHANGE_NAME, DIALOG_STYLE_INPUT, ""HEX_RED"Zmień nazwę drzwi", ""HEX_RED"Nazwa jest zbyt krótka!\n"HEX_WHITE"Wpisz poniżej nową nazwę:\n", "Zmień", "Anuluj");
		...
		case D_DOOR_VW:
		{
			...
			{
				if(Isnull(inputtext))
				return SendClientMessage(playerid, COLOR_GRAY, "Niepoprawna wartość.");
				...
				new msg[128]; format(msg, sizeof(msg), "Wyjściowy Virtual World tych drzwi został poprawnie zmieniony na: %d", vw);
				return SendClientMessage(playerid, COLOR_GRAY, msg);
				...
				if(VehicleCache[vuid][vID] != 0)
				{
					for(new i; i<=GetPlayerPoolSize(); i++)
					{
						if(IsPlayerConnected(i))
						{
							...
							{
								return SendClientMessage(playerid, COLOR_GRAY, "Ktoś znajduje się w tym pojeździe zatem nie możesz go odspawnować.");
							}
						}
					}
		...
		case D_CHANGE_GROUP_COLOR:
		{
			if(response)
			{
				...
				new msg[64]; format(msg, sizeof(msg), "{%s}Kolor grupy został zmieniony pomyślnie!", GroupCache[guid][gColor]);
			...
			if(response)
			{
				...
				if(uid)
				{
					if(PlayerCache[playerid][pGroup] == guid)
					{
						...
						{
							return ShowDialogInfo(playerid, "Lider grupy nie nadał Ci uprawnień do zmiany opcji członków.");
						}
					}
				...
				new info[128];
				new list[725] = "#\tCzłonek\tUprawnienia\n";
				new count;
				for(new i=pPage[playerid]; i<LastUID; i++)
				{
					if(i == 0)
			...
			if(response)
			{
				...
				switch(option)
				{
					...
					case 2:
					{
						...
						{
							if(!PlayerCache[playerid][pGroupAdmin])
							return ShowDialogInfo(playerid, "Lider tej grupy nie nadał Ci odpowiednich\nuprawnień do zarządzania nią.");
						}
						else if(PlayerCache[playerid][pGroup2] == guid)
						{
							if(!PlayerCache[playerid][pGroupAdmin2])
		...
		case D_DOOR_GROUP:
		{
			if(response)
			{
				...
				format(text, sizeof(text), "> Drzwi zostały pomyślnie podpisane pod grupę %s (UID: %d). Użyj /drzwi [opcje] by ustawić opłatę za przejście!", GroupCache[DoorCache[uid][dGroupUID]][gName], DoorCache[uid][dGroupUID]);
		...
		case D_CHANGE_URL:
		{
			if(response)
			{
				...
				new msg[128];
				format(msg, sizeof(msg), "* %s zmienia płytę w systemie nagłaśniającym audio.", strreplace(ReturnPlayerName(playerid), '_', ' '));
		...
		case D_BOOMBOX:
		{
			if(response)
			{
				...
				ApplyAnimation(playerid, "bomber", "bom_plant", 4.1, 0, 0, 0, 0, 0, 0);
				new str[128]; format(str, sizeof(str), "kładzie %s na ziemie oraz wybiera stację radiową.", ItemCache[uid][iName]);
				...
				switch(option)
				{
					...
					case 9:
					{
						if(!DoorCache[uid][dAlarm])
						{
							...
							return ShowDialogInfo(playerid, ""HEX_WHITE"Alarm został zakupiony!\nOdpowiednie służby zostaną poinformowane podczas próby włamania do budynku.\nWłamanie zostanie wykryte również przy strzelaniu w budynku z "HEX_DARKRED"głośnej "HEX_WHITE"broni palnej.");
						}
		...
		case D_PASS:
		{
			if(response)
			{
				...
				new query[1024];
				format(query, sizeof(query), "INSERT INTO players (hash, salt,name, gender, borndate) VALUES ('%s', '%s', '%s', '%d', '%d')", hash, salt,RegisterCache[playerid][rName], RegisterCache[playerid][rSex], RegisterCache[playerid][rBornDate]);
				mysql_query(query);
		...
		case D_ITEMS:
		{
			...
			if(response)
			{
				...
				return ShowDialogInfo(playerid, "Nie możesz tego zrobić poczas trwania BW.");
		...
		case D_V_OFFER:
		{
			...
			if(response)
			{
				...
				if(senderid == -1)
				return SendClientMessage(playerid, COLOR_GRAY, "Gracz wylogował się lub opuścił grę.");
				if(!IsPlayerInRangeOfPlayer(playerid, senderid, 5.0))
				return SendClientMessage(playerid, COLOR_GRAY, "Gracz oddalił się.");
				if(!IsPlayerInAnyVehicle(senderid))
				return SendClientMessage(playerid, COLOR_GRAY, "Gracz opuścił swój pojazd.");
				if(PlayerCache[playerid][pCash] < price)
				{
					TextDrawForPlayerEx(senderid, 1, msg, 3000);
					return SendClientMessage(playerid, COLOR_GRAY, "Nie posiadasz przy sobie aż tylu pieniędzy.");
				}
				if(Isnull(inputtext))
				{
					SendClientMessage(playerid, COLOR_GRAY, "Wpisz POTWIERDZAM w dialogu, jeśli chcesz zaakceptować ofertę w jej obecnym stanie.");
					new info[1025]; format(info, sizeof(info), "Informacje o pojeździe:\n\nMarka: %s\nPrzebieg: %dkm\nPaliwo: %d/100\nStan techniczny: %fHP\n\nnCena: "HEX_GREEN"$%d\n\n"HEX_WHITE"Wpisz poniżej "HEX_DARKRED"POTWIERDZAM"HEX_WHITE", jeśli chcesz zaakceptować tę ofertę w jej obecnym stanie.",
		...
		case D_V_SELL:
		{
			...
			{
				if(!IsPlayerInAnyVehicle(playerid))
				return SendClientMessage(playerid, COLOR_GRAY, "Nie znajdujesz się w żadnym pojeździe.");
				new vid = GetPlayerVehicleID(playerid);
				new vuid = GetVehicleUID(vid);
				if(vuid != pVal[playerid])
				return SendClientMessage(playerid, COLOR_GRAY, "Opuściłeś(aś) swój pojazd");
				new price = strval(inputtext);
				if(price < 1)
				return ShowPlayerDialog(playerid, D_V_SELL, DIALOG_STYLE_INPUT, "Sprzedaj pojazd", ""HEX_DARKRED"Minimalna cena za, którą możesz sprzedać swój pojazd to 1$.\n"HEX_WHITE"Wpisz poniżej cenę, za jaką chcesz sprzedać swój pojazd:", "Sprzedaj", "Anuluj");
		...
		case D_V_NEAR_PLAYERS:
		{
			if(response)
			{
				...
				return ShowPlayerDialog(playerid, D_V_SELL, DIALOG_STYLE_INPUT, "Sprzedaj pojazd", ""HEX_WHITE"Wpisz poniżej cenę, za jaką chcesz sprzedać swój pojazd:", "Gotowe", "Anuluj");
			}
			return 1;
		}
		case D_NEARBY_ITEMS:
		...
		case D_USE_WEAP:
		{
			if(response)
			{
				...
				sscanf(data, "p<|>iiiiis[128]", val, val2, val3, val4, active, name);
				...
				else
				{
					...
					format(msg, sizeof(msg), "wyciąga %s %s.", name, inputtext);
		...
		case D_STACK:
		{
			if(response)
			{
				...
				return SendClientMessage(playerid, COLOR_GRAY, "Opuściłeś(aś) budynek grupy lub został on odpisany.");
				new count, customprice;
				if(sscanf(inputtext, "ii", count, customprice))
				{
					ShowPlayerDialog(playerid, D_STACK, DIALOG_STYLE_INPUT, "Parametry produktu", "Wpisz poniżej ilość tego produktu do zamówienia oraz\nwłasną cenę, za jaką klient będzie musiał zapłacić za kupno jednego takiego produktu.\nWłasna, ustalona cena musi być większa\n\
					niż cena hurtowa.\n\nFormat: [ilość] [cena za którą zapłaci każda osoba kupująca za jeden dany przedmiot]\n\
					Przykład: 10 100", "Zamów", "Anuluj");
					return SendClientMessage(playerid, COLOR_GRAY, "Minimalna ilość to 1, a maksymalna to 100 za jednym razem.");
				}
				if(count < 1 || count > 100)
				{
					SendClientMessage(playerid, COLOR_GRAY, "Minimalnie możesz zawówić jeden produkt grupowy!");
					ShowPlayerDialog(playerid, D_STACK, DIALOG_STYLE_INPUT, "Parametry produktu", "Wpisz poniżej ilość tego produktu do zamówienia oraz\nwłasną cenę, za jaką klient będzie musiał zapłacić za kupno jednego takiego produktu.\nWłasna, ustalona cena musi być większa\n\
					niż cena hurtowa.\n\nFormat: [ilość] [cena za którą zapłaci każda osoba kupująca za jeden dany przedmiot]\n\
					Przykład: 10 100", "Zamów", "Anuluj");
					return SendClientMessage(playerid, COLOR_GRAY, "Minimalna ilość to 1, a maksymalna to 100 za jednym razem.");
				...
				{
					SendClientMessage(playerid, COLOR_GRAY, "Cena dla jednego produktu musi być równa lub większa cenie hurtowej.");
					ShowPlayerDialog(playerid, D_STACK, DIALOG_STYLE_INPUT, "Parametry produktu", "Wpisz poniżej ilość tego produktu do zamówienia oraz\nwłasną cenę, za jaką klient będzie musiał zapłacić za kupno jednego takiego produktu.\nWłasna, ustalona cena musi być większa\n\
					niż cena hurtowa.\n\nFormat: [ilość] [cena za którą zapłaci każda osoba kupująca za jeden dany przedmiot]\n\
				...
				return SendClientMessage(playerid, COLOR_GRAY, "Grupa nie posiada aż tylu funduszy w banku by zamówić ten produkt w takiej ilości.");
			...
			if(response)
			{
				...
				switch(phone_number)
				{
					case 777:
					{
						...
						if(!dooruid)
						return ShowDialogInfo(playerid, "Nie znajdujesz się w budynku żadnej grupy.");
						new groupuid = DoorCache[dooruid][dGroupUID];
						if(PlayerCache[playerid][pGroup] == groupuid)
						{
							if(!PlayerCache[playerid][pGroupProducts])
							return ShowDialogInfo(playerid, "Nie posiadasz uprawnień do zamawiania produktów w tej grupie.");
						}
						...
						if(!strlen(list))
						return ShowPlayerDialog(playerid, D_INFO, DIALOG_STYLE_MSGBOX, "Brak produktów", ""HEX_WHITE"Dla tej grupy nie został przypisane żadne produkty do zamówienia.\nSkontaktuj się z Administracją.", "zamknij", "");
					...
					case 911:
					{
						...
						if(Isnull(list))
						return ShowDialogInfo(playerid, "Żadne organizacje porządkowe nie zostały jeszcze utworzone.");
						return ShowPlayerDialog(playerid, D_911, DIALOG_STYLE_LIST, "Służby porządkowe", list, "Zadzwoń", "Anuluj");
					}
					case 555:
					{
						...
						if(Isnull(list))
						return ShowDialogInfo(playerid, "Aktualnie nie ma nikogo na służbie żadnego z biznesów.");
				...
				{
					ShowDialogContacts(playerid); return TextDrawForPlayerEx(playerid, 1, "Nie możesz zadzwonić do samego siebie.", 5000);
				}
				for(new i; i<MAX_ITEMS; i++)
				{
					if(ItemCache[i][iType] == 2)
					{
						if(ItemCache[i][iVal] == phone_number-50000)
						{
							...
							return SendClientMessage(playerid, COLOR_GREEN, "#Operator Komórkowy: "HEX_WHITE"Abonent jest nieosiągalny.");
						}
					}
				}
				SendClientMessage(playerid, COLOR_GREEN, "#Operator Komórkowy: "HEX_WHITE"Wprowadzono nieprawdiłowy numer telefonu.");
				return ShowDialogContacts(playerid);
			}
			return ShowDialogContacts(playerid);
		}
			...
			if(response)
			{
				switch(option)
				{
					...
					case 3:
					{
						...
						if(Isnull(list))
						return ShowDialogInfo(playerid, "Nie posiadsz żadnych kontaktów w swoim telefonie.");
						return ShowPlayerDialog(playerid, D_REMOVE_CONTACT, DIALOG_STYLE_LIST, "Wybierz kontakt do usunięcia", list, "Usuń", "Anuluj");
					}
				}
			}
			return ShowDialogPhone(playerid);
		...
		case D_ADD_CONTACT:
		{
			...
			return SendClientMessage(playerid, COLOR_GRAY, "Oferujący opuścił grę.");*/
			new senderid;
			for(new i; i<=GetPlayerPoolSize(); i++)
			{
				if(IsPlayerConnected(i))
		...
		case D_VCARD_ACCEPT:
		{
			...
			if(response)
			{
				...
				return SendClientMessage(playerid, COLOR_GRAY, "Wybrany gracz nie posiada telefonu lub jest on wyłączony.");
			}
			else
			{
				if(targetid != -1)
			...
			if(response)
			{
				...
				switch(option)
				{
					...
					case 3:
					{
						...
						list = ""HEX_RED"-1\tWyłącz animację na przycisk\n";
...
stock ShowDialogVCardAccept(playerid)
{
	...
	new str[256];
	format(str, sizeof(str), ""HEX_YELLOW"%s"HEX_WHITE" wysyła Ci swój numer.\nCzy chcesz dodać go jako kontakt do swojego telefonu?", strreplace(ReturnPlayerName(senderid), '_', ' '));
...
stock ShowDialogVCard(playerid)
{
	...
	if(!strlen(list))
	return ShowDialogInfo(playerid, "Nie znaleziono żadnych graczy w Twoim pobliżu.");
...
{
	return ShowPlayerDialog(playerid, D_GROUP_PAYDAY, DIALOG_STYLE_INPUT, "Opcje grupy > Domyślna wypłata", "Wpisz poniżej domyślną wypłatę jaką będą dostawać nowi członkowie grupy.\nMożesz zmienić wypłatę poszczególnym osobom wybierając ją z listy członków.\n\
	To ile możesz ustawić domyślnej wypłaty lub wypłaty dla danego członka jest zależne od rodzaju Twojej grupy:", "Gotowe", "Anuluj");
}
...
{	
	...
	if(pTalking[playerid] == -1)
	{
		SendClientMessage(playerid, COLOR_GRAY, "Rozmówca nie odebrał telefonu.");
		SetPlayerSpecialAction(playerid, SPECIAL_ACTION_STOPUSECELLPHONE);
	}
	return 1;
}
...
stock LoginPlayer(playerid)
{
	...
	new str[128];
	format(str, sizeof(str), "> Witaj, %s! "HEX_GRAY"(UID: %d, ID: %d)"HEX_WHITE". Życzymy miłej gry!", ReturnPlayerName(playerid), PlayerCache[playerid][pUID], playerid);
	ClearChat(playerid);
...
public OnPlayerCommandReceived(playerid, cmd[], params[], flags)
{
	...
	{
		if(PlayerCache[playerid][pBW_Time])
		return SendClientMessage(playerid, COLOR_GRAY, "Nie możesz tego zrobić podczas trwania BW.");
	...
	if(pBus[playerid])
	return !SendClientMessage(playerid, COLOR_GRAY, "Nie możesz teraz tego zrobić.");
...
public TurnOnEngine(vuid)
{
	...
	if(VehicleCache[vuid][vBanCost])
	{
		...
		new msg[256];
		format(msg, sizeof(msg), "Nie możesz uruchomić pojazdu bo posiada on blokadę na koło.\nKoszt zdjęcia: $%d\n\nNotka: %s\nSkontaktuj się z policją by odblokować koło.", VehicleCache[vuid][vBanCost],
...
{
	new list[525]; format(list, sizeof(list), "1\tZmień kolor grupy\n2\t%sPrzełącz czat OOC\n3\t%sPrzełącz czat IC\n4\tDomyślna wypłata: "HEX_BLUE"$%d", GroupCache[groupuid][gChatOOC] ? (""HEX_GREEN"") : (""HEX_RED""), GroupCache[groupuid][gChatIC] ? (""HEX_GREEN"") : (""HEX_RED""), GroupCache[groupuid][gPayDay]);
	...
	else if(keys & KEY_SPRINT && pTrainingState[playerid] != TRAINING_STATE_DOWN && pTrainingState[playerid] != TRAINING_STATE_DONE)
	{
		...
		if(pHoldingBar[playerid] == 8)
		{
			...
			if(pTrainingExtrude[playerid] == 100)
			{
				...
				RemovePlayerAttachedObject(playerid, ATTACH_SLOT_CASK);
				new info[256]; format(info, sizeof(info), "Zakończono trening.\nNastępny trening możesz odbyć za 16h od momentu ukończenia tego treningu.\n\nTwoja siła zwiększyła się o %dj!\nAktualna siła: %dj.", last_strenght, PlayerCache[playerid][pStrenght],
				PlayerCache[playerid][pStrenght]);
				for(new i; i<MAX_ITEMS; i++)
				{
					if(ItemCache[i][iState] == 0)
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	...
	else if(newkeys & KEY_YES)
	{
		...
		if(objid)
		{
			new query[256], data[1024], ownerType, owner, Float:x, Float:y, Float:z, Float:rx, Float:ry, 
		...
		/*if(ObjectCache[ouid][oModel] == 2629)
		{
					...
					if(ItemCache[i][iOwner] == PlayerCache[playerid][pUID])
					{
						if(ItemCache[i][iType] == 16)
						{
							if(ItemCache[i][iActive])
							{
								...
								return ShowDialogInfo(playerid, "Następny trening możesz rozpocząć 16h od momentu skończenia ostatniego treningu.");
			...
			return ShowDialogInfo(playerid, "Nie posiadasz karnetu siłowego lub jest on nieaktywny.\nZdobądź karnet lub aktywuj go i wtedy zacznij ćwiczyć,");
		}*/
		/*if(ObjectCache[ouid][oModel] == 1257)
		{
			if(IsPlayerInRangeOfPoint(playerid, 5.0, ObjectCache[ouid][oX],  ObjectCache[ouid][oY],  ObjectCache[ouid][oZ]))
			{
				...
				if(count)
				{
					SendPlayerMe(playerid, "sprawdza listę przystanków autobusowych.");
...
stock RPFormatText(message[], hexme[], hexnormal[])
{
	new hex[128]; format(hex, sizeof(hex), hexme); // hexme
...
public OnPlayerText(playerid, text[])
{
	...
	if(GetTickCount() - pTick[playerid] <= 250)
	{
		return !SendClientMessage(playerid, COLOR_GRAY, "Odczekaj chwilę przed ponownym wysłaniem wiadomości.");
	}
	pTick[playerid] = GetTickCount();
	if(ac[playerid])
	{
	...
	if(text[0] == '!')
	{
		...
		switch(slot)
		{
			case 1:
			{
				...
				return !SendClientMessage(playerid, COLOR_GRAY, "Słuchawka dla tej grupy jest wyłaczona.");
				groupuid = PlayerCache[playerid][pGroup];
			}
			case 2:
			{
		...
		new msg[128];
		new msg2[128];
		format(msg2, sizeof(msg2), "%s (słuchawka): %s", strreplace(ReturnPlayerName(playerid), '_', ' '), playermsg);
		new vw = GetPlayerVirtualWorld(playerid);
		for(new i; i<=GetPlayerPoolSize(); i++)
		{
			if(IsPlayerConnected(i))
					...
					else
					{
						...
						if(GetPlayerVirtualWorld(i) == vw)
						{
							if(IsPlayerInRangeOfPoint(i, 1, X, Y, Z))
							{
								...
								{
									SendClientMessage(i, ME_SHADE_1, "Twoja postać jest nieprzytomna, więc nie może słyszeć wypowiedzi innych osób.");
									continue;
								}
							...
							else if(IsPlayerInRangeOfPoint(i, 2, X, Y, Z))
							{
								if(PlayerCache[i][pBW_Time] > 0)
								{
									SendClientMessage(i, ME_SHADE_2, "* Twoja postać jest nieprzytomna, więc nie może słyszeć wypowiedzi innych osób.");
									continue;
								}
							...
							else if(IsPlayerInRangeOfPoint(i, 3, X, Y, Z))
							{
								if(PlayerCache[i][pBW_Time] > 0)
								{
									SendClientMessage(i, ME_SHADE_3, "* Twoja postać jest nieprzytomna, więc nie może słyszeć wypowiedzi innych osób.");
									continue;
								}
							...
							else if(IsPlayerInRangeOfPoint(i, 6, X, Y, Z))
							{
								if(PlayerCache[i][pBW_Time] > 0)
								{
									SendClientMessage(i, ME_SHADE_4, "* Twoja postać jest nieprzytomna, więc nie może słyszeć wypowiedzi innych osób.");
									continue;
								}
							...
							else if(IsPlayerInRangeOfPoint(i,12, X, Y, Z))
							{
								if(PlayerCache[i][pBW_Time] > 0)
								{
									SendClientMessage(i, ME_SHADE_5, "* Twoja postać jest nieprzytomna, więc nie może słyszeć wypowiedzi innych osób.");
									continue;
								}
							...
							else if(IsPlayerInRangeOfPoint(i, 18, X, Y, Z))
							{
								if(PlayerCache[i][pBW_Time] > 0)
								{
									SendClientMessage(i, ME_SHADE_6, "* Twoja postać jest nieprzytomna, więc nie może słyszeć wypowiedzi innych osób.");
									continue;
								}
...
stock ShowDialogAnim(playerid)
{
	...
	return ShowPlayerDialog(playerid, D_ANIMS, DIALOG_STYLE_LIST, "Animacje (wybierz aby użyć)", list, "Wybierz", "Anuluj");
}
			...
			for(new i; i<=GetPlayerPoolSize(); i++)
			{
				if(IsPlayerConnected(i))
				{
					if(pLogged[i])
					{
						...
						SendClientMessage(i, COLOR_RED, "< Wybiła północ. Możliwość otrzymania PD została wznowiona >");
					}
				}
			}
		}
			...
			if(pLogged[i])
			{
					...
					if(PlayerCache[i][pGroup] == guid)
					{
						...
						if(PlayerCache[i][pGroupDuty] == 31 && !PlayerCache[i][pGroupPayDay] && GroupCache[guid][gType] != 0)
						{
							...
							else
							{
								...
								format(query,sizeof(query), "** Otrzymujesz $%d wypłaty **", PlayerCache[i][pGroupReward]);
				...
				if(PlayerCache[i][pBW_Time])
				{
					...
					if(PlayerCache[i][pBW_Time] == 0)
					{
						...
						ShowDialogInfo(i, "Twoja postać ocknęłą się po utracie przytomności.");
...
{
	if(PlayerCache[playerid][pBW_Time])
	return ShowDialogInfo(playerid, "Nie możesz teraz tego zrobić.");
	for(new i; i<MAX_ITEMS; i++)
	{
		if(ItemCache[i][iState] == ITEM_STATE_DRIVING)
		{
...
cmd:report (playerid, params[])
{
	...
	new msg[128]; format(msg, sizeof(msg), ""HEX_RED"%s(%d) wysłał raport na gracza %s(%d):", ReturnPlayerName(playerid), playerid, ReturnPlayerName(targetid), targetid);
	for(new i; i<=GetPlayerPoolSize(); i++)
	{
		if(IsPlayerConnected(i))
		{
...
cmd:unbw (playerid, params[])
{
	...
	new msg[64];
	format(msg, sizeof(msg), "%s ściąnął Ci BW.", ReturnPlayerName(playerid));
...
cmd:joty (playerid, params[])
{
	...
	UpdatePlayerName(targetid);
	new msg[128];
	format(msg, sizeof(msg), "%s zmienił twoją wartość Jotów na: %d", ReturnPlayerName(playerid), j);
	SendClientMessage(targetid, COLOR_GRAY, msg);
	format(msg, sizeof(msg), "Zmienił wartość Jotów %s na: %d", ReturnPlayerName(targetid), j);
	SendClientMessage(playerid, COLOR_GRAY, msg);
	return 1;
}
...
cmd:hp (playerid, params[])
{
	...
	if(playerid == targetid)
	{
		format(msg, sizeof(msg), "Zmieniłeś(aś) swoją wartość HP na: %f", HP);
		SendClientMessage(playerid, COLOR_GRAY, msg);
	}
	else
	{
		format(msg, sizeof(msg), "%s zmienił(a) twoją wartość HP na: %f", ReturnPlayerName(playerid), HP);
		SendClientMessage(targetid, COLOR_GRAY, msg);
		format(msg, sizeof(msg), "Zmieniono wartość HP gracza %s na: %f", ReturnPlayerName(targetid), HP);
		SendClientMessage(playerid, COLOR_GRAY, msg);
	}
	return 1;
}
...
cmd:alevel (playerid, params[])
{
	...
	UpdatePlayerName(targetid);
	new msg[128];
	format(msg, sizeof(msg), "%s zmienił Twój poziom rangi na: %d", ReturnPlayerName(playerid), rank);
	SendClientMessage(targetid, COLOR_GRAY, msg);
...
stock BlockPlayer(playerid, adminname[], reason[])
{
	...
	PenalityTimer = SetTimer("HidePenalityTextDraw", 15000, false);
	format(str, sizeof(str), "> Otrzymałeś/aś karę(Blokada postaci) od %s. Powód: %s", adminname, reason);
	KickFix(playerid);
...
cmd:warn (playerid, params[])
{
	...
	PenalityTimer = SetTimer("HidePenalityTextDraw", 15000, false);
	format(str, sizeof(str), "> Otrzymałeś karę(ostrzeżenie) od %s. Powód: %s", ReturnPlayerName(playerid), reason);
...
CMD:gname (playerid, params[])
{
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Nowa nazwa grupy musi pomieścić się pomiędzy 3-32 znakami.");
...
CMD:wejscie (playerid, params[])
{
	...
	new Float:X, Float:Y, Float:Z; GetPlayerPos(playerid, X, Y, Z);
	CreateDoor(playerid, GetPlayerVirtualWorld(playerid), 1, X, Y, Z);
	new msg[128]; format(msg, sizeof(msg), "Utworzono przejście (UID przejścia: %d)", LastdUID-1);
...
CMD:wyjscie (playerid, params[])
{
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Tip: /wyjscie [UID drzwi - przejścia] by ustawić pozycję wyjściową w, której stoisz.");
	...
	new msg[256]; format(msg, sizeof(msg), "Zmieniono pozycję wyjściową drzwi UID: %d na bieżącą pozycje postaci (oraz VW).", duid);
...
CMD:dname (playerid, params[])
{
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Niepoprawna wartość UID drzwi.");
	DoorCache[duid][dName] = name;
...
cmd:zamknij (playerid, params[])
{
	...
	{
		sscanf(str, "p<|>iiiiffffffii", playerUID, groupUID, open, explodeTime, insX, insY, insZ,
		outX, outY, outZ, insVW, outVW);
		mysql_free_result();
	}
	else
	return SendClientMessage(playerid, COLOR_GRAY, "Nie znajdujesz się przy żadnych drzwiach.");
...
cmd:w (playerid, params[])
{
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Podczas trwania BW, wiadmości możesz wysyłać tylko na krótką odległość.");
	if(!pPriv[targetid])
	return SendClientMessage(playerid, COLOR_GRAY, "Ten gracz ma wyłączone wiadomości prywatne.");
	new str[256];
...
cmd:cash (playerid, params[])
{
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Ten gracz nie jest zalogowany.");
	new msg[256];
	format(msg, sizeof(msg), "(( %s %s[%d] nadał graczowi %s[%d] $%d gotówki. ))", GetRankName(PlayerCache[playerid][pLevel]), ReturnPlayerName(playerid),playerid, ReturnPlayerName(targetid),targetid, money);
	...
	for(new i=0;i<=GetPlayerPoolSize(); i++)
	{
		if(IsPlayerConnected(i))
		{
			if(PlayerCache[i][pUID] == PlayerMessageUID[playerid])
			{
				...
				return SendClientMessage(playerid, COLOR_GRAY, "Podczas trwania BW, wiadmości możesz wysyłać tylko na krótką odległość.");
				if(!pPriv[playerid])
				return SendClientMessage(playerid, COLOR_GRAY, "Nie możesz wysłać wiadomości kiedy masz wyłączone PW.");
				if(!pPriv[i])
				return SendClientMessage(playerid, COLOR_GRAY, "Ten gracz ma wyłączone wiadomości prywatne.");
...
{
	new query[256];
	format(query, sizeof(query), "INSERT INTO items (owner, name, type, val, val2, val3, val4, groupOwner) VALUES ('%d', '%s', '%d', '%d', '%d', '%d', '%d', '%d')",
	owneruid, name, type, val, val2, val3, val4, group);
	mysql_query(query);

	mysql_query("SELECT Max(uid) FROM items");
...
{	
	if(pCuffed[playerid])
	return ShowDialogInfo(playerid, "Nie możesz tego zrobić będąc skutym.");

	if(PlayerCache[playerid][pBW_Time])
	return ShowDialogInfo(playerid, "Nie możesz tego teraz zrobić.");
	...
	format(query, sizeof(query), "SELECT uid, type, val, val2, val3, val4, name, active FROM items WHERE owner = '%d' AND state = '%d'", PlayerCache[playerid][pUID], ITEM_STATE_EQ);
	...
	new uid, type, val, val2, val3, val4, name[128], active, list[256];

	while(mysql_fetch_row(data))
	{
		sscanf(data, "p<|>iiiiiis[128]i",
		uid, type, val, val2, val3, val4, name, active);
...
CMD:c (playerid, params[])
{
	...
	new text[128];
	if(sscanf(params, "s[128]", text))
	return SendClientMessage(playerid, COLOR_GRAY, "Poprawne użycie: /c [szept na odleg��ość 1m!]");	
	new Float:X, Float:Y, Float:Z; GetPlayerPos(playerid, X, Y, Z);
...
CMD:ai(playerid, params[])
{
	...
	{
		new query[128];
		format(query, sizeof(query), "SELECT * FROM items WHERE owner = '%d'", PlayerCache[playerid][pUID]);
		mysql_query(query);
		mysql_store_result();
		if(mysql_num_rows()>=20)
		return SendClientMessage(playerid, COLOR_GRAY, "W ekwipunku nie zmieści się aż tyle przedmiotów.");

		new type, val, val2, val3, val4, group, name[128];
		if(sscanf(params, "iiiiiis[128]", type, val, val2, val3,val4, group, name))

		return SendClientMessage(playerid, COLOR_GRAY, "Poprawne użycie: /ai [Typ] [wartość 1] [wartość 2] [wartość 3] [wartość 4] [UID grupy] [Nazwa]");
		CreateItem(PlayerCache[playerid][pUID], type, val, val2, val3,val4, group, name);
		...
		format(name, sizeof(name), "Stworzono przedmiot (UID: %d)", uid);
...
CMD:aprodukt(playerid, params[])
{
	if(PlayerCache[playerid][pLevel] == ADMINISTRATION || pDuty[playerid] == 16 || PlayerCache[playerid][pLevel] == DEVELOPER)
	{
		...
		return SendClientMessage(playerid, COLOR_GRAY, "Poprawne użycie: /aprodukt [typ grupy] [cena za sztuke] [Typ przedmiotu] [wartość 1] [wartość 2] [wartość 3] [wartość 4] [UID grupy] [Nazwa]");
		...
		format(name, sizeof(name), "Stworzono produkt dla rodzaju grupy: "HEX_BLUE"%s"HEX_WHITE" o nazwie: %s z ceną %d za sztukę.",GetGroupType(ItemCache[MAX_ITEMS-1][iOwner]), ItemCache[MAX_ITEMS-1][iName], ItemCache[MAX_ITEMS-1][iCost]);
		SendClientMessage(playerid, -1, name);
	}
	return 1;
}
...
CMD:kup (playerid, params[])
{
	...
	new list[1025] = "#\tNazwa\tCena\tIlość\n", info[128], count;
	for(new i; i<MAX_ITEMS; i++)
	{
		if(ItemCache[i][iState] == ITEM_STATE_PRODUCT)
		{
...
stock UseItem(playerid, itemuid)
{
	...
	new query[128];
	format(query, sizeof(query), "SELECT groupOwner, state, type, val, val2, val3, val4, owner, active, name FROM items WHERE uid = '%d' LIMIT 1", itemuid);
	mysql_query(query);
	...
	if(mysql_fetch_row(data))
	{
		sscanf(data, "p<|>iiiiiiiiis[128]", groupowner, status, type, val, val2, val3, val4, owner, active, name);
	}
	...
	{
		if(owner != PlayerCache[playerid][pUID])
		return TextDrawForPlayerEx(playerid, 1, "Ten przedmiot nie nalezy do Ciebie.", 3000);
	}
	...
	if(groupowner)
	{
		if(PlayerCache[playerid][pGroup] == groupowner)
		{

		}
		else if(PlayerCache[playerid][pGroup2] == groupowner)
		{

		}
		else if(PlayerCache[playerid][pGroup3] == groupowner)
		{

		}
		else
		{
			return ShowDialogInfo(playerid, "Ten przedmiot jest powiązany z konkretną grupą i tylko ona może z niego korzystać.");
		}
	}
	switch(type)
	{
		case 0: return ShowDialogInfo(playerid, "Ten przedmiot służy do gry RolePlay i nie ma żadnych zastosowań skryptowych.");
		case 1:
		{
			if(val2 == 0)
			return SendClientMessage(playerid, COLOR_GRAY, "W tej broni skończyła się amunicja.");
			
			format(query, sizeof(query), "SELECT * FROM items WHERE owner = '%d' AND type = '1' AND active = '1' AND uid != '%d' LIMIT 1", PlayerCache[playerid][pUID], itemuid);
			mysql_query(query);

			mysql_store_result();

			if(mysql_num_rows())
			return SendClientMessage(playerid, COLOR_GRAY, "Masz już wyjętą jakąś broń.");
		...
		case 3:
		{
			...
			format(query, sizeof(query), "SELECT * FROM items WHERE type = '3' AND active = '1' AND uid != '%d' LIMIT 1",
			itemuid);
			mysql_query(query);
			mysql_store_result();
			if(mysql_num_rows())
		...
		{
			new str[256];
			format(str, sizeof(str), "spożywa %s", name);
			...
			switch(val3)
			{
				...
				case 2:
				{
					...
					return ShowDialogInfo(playerid, "Wspomagacz na siłownie został przez Ciebie użyty.\nW ciągu godziny udaj się poćwiczyć na siłownie by zwiększyć zdobytą siłę dwa razy.");
				}
				default:
				{
		...
		case 14:
		{
			...
			if(vid != INVALID_VEHICLE_ID)
			{
				...
				UnSpawnVehicle(vuid);
				SpawnVehicle(vuid);
				ShowDialogInfo(playerid, "Pomyślnie zamontowano rejstrację pojazdu.");
		...
		case 15:
		{
			...
			if(Isnull(list))
			return ShowDialogInfo(playerid, "Nie znaleziono pasujących przedmiotów.");
			pVal[playerid] = itemuid;
		...
		case 16:
		{
				...
				if(ItemCache[i][iType] == 16)
				{
					if(ItemCache[i][iUID] != itemuid)
					{
						if(ItemCache[i][iState] == 0)
						{
							...
							{
								if(ItemCache[i][iActive])
								return SendClientMessage(playerid, COLOR_GRAY, "Nie możesz mieć aktywnych więcej karnetów siłowych.");
							}
						}
					}
				}
			...
			for(new i; i<LastdUID; i++)
			{
				if(!DoorCache[i][dDestroyed])
				{
					...
					{
						ItemCache[itemuid][iActive] = 1;
						return ShowDialogInfo(playerid, "Karnet siłowy został aktywowany.\nNajedź kamerą na przyrząd do ćwiczeń i naciśnij 'Y' by zacząć ćwiczyć");
					}
				}
			}
			return ShowDialogInfo(playerid, "Ten karnet może być aktywowany tylko w siłowni w, której został zakupiony.");
		}
		case 17:
		{
			if(GroupCache[pDuty[playerid]][gType] == 1 || GroupCache[pDuty[playerid]][gType] == 16)
							...
							if(IsPlayerInRangeOfPoint(playerid, 2.5, DoorCache[i][dInsX], DoorCache[i][dInsY], DoorCache[i][dInsZ]))
							{
								if(GetPlayerVirtualWorld(playerid) == DoorCache[i][dInsVW])
								{
									if(GetPlayerVirtualWorld(playerid) == DoorCache[i][dOutVW])
									{
										...
										new str[64]; format(str, sizeof(str), "podkłada %s.", ItemCache[itemuid][iName]);
...
{
	new header[64]; format(header, sizeof(header), "Połączenie od numeru: %d", GetPlayerPhoneNumber(pCalling[playerid])+50000);
...
public OnPlayerWeaponShot(playerid, weaponid, hittype, hitid, Float:fX, Float:fY, Float:fZ)
{
	...
	if(weaponid != 23)
	{
		if(GetPlayerVirtualWorld(playerid) != 0)
		{
			...
			{
				new msg[128];
				format(msg, sizeof(msg), "** Padły strzały! Zgłoszenie zostało oznaczone na radarze. **");
	...
	if(WeaponCache[playerid][wcVal2] == 1)
	{
		...
		return SendClientMessage(playerid, COLOR_GRAY, "W broni skończyła się amunicja.");
	}
...
CMD:g (playerid, params[])
{
	if(!Isnull(params))
	{
		...
		switch(slot)
		{
			case 1: if(PlayerCache[playerid][pGroup] == 0) return SendClientMessage(playerid, COLOR_GRAY, "Pod wybranym slotem nie znajduje się żadna grupa.");
			case 2: if(PlayerCache[playerid][pGroup2] == 0) return SendClientMessage(playerid, COLOR_GRAY, "Pod wybranym slotem nie znajduje się żadna grupa.");
			case 3: if(PlayerCache[playerid][pGroup3] == 0) return SendClientMessage(playerid, COLOR_GRAY, "Pod wybranym slotem nie znajduje się żadna grupa.");
		...
		if(!Isnull(sub))
		{
			if(!strcmp(sub, "wyplac", true))
			{
				new guid;
				if(targetid < 1)
				return SendClientMessage(playerid, COLOR_GRAY, "Z grupy minimalnie wypłacić możesz 1$.");
				switch(slot)
				{
					case 1:	
					{
						if(!PlayerCache[playerid][pGroupAdmin])
						return SendClientMessage(playerid, COLOR_GRAY, "Nie posiadasz uprawnień do zarządzania tą grupą.");
						guid = PlayerCache[playerid][pGroup];
						if(GroupCache[guid][gBank] < targetid)
						return SendClientMessage(playerid, COLOR_GRAY, "Na koncie grupy nie znajduje się aż tyle pieniędzy.");
					}
					case 2:	
					{
						if(!PlayerCache[playerid][pGroupAdmin2])
				...
				new msg[128]; format(msg, sizeof(msg), "#Bank "HEX_WHITE"Na konto wypłynęła kwota $%d.", targetid);
				return SendClientMessage(playerid, COLOR_GREEN, msg);
			}
			else if(!strcmp(sub, "wplac", true))
			{
				if(targetid < 1 || targetid > 10000)
				return SendClientMessage(playerid, COLOR_GRAY, "Dotacja dla grupy musi znajdować się w przedziale 1-10000.");
				if(PlayerCache[playerid][pBank] < targetid)
				return SendClientMessage(playerid, COLOR_GRAY, "Nie posiadasz tyle pieniędzy w banku.");
				PlayerCache[playerid][pBank] -= targetid;
			...
			else if(!strcmp(sub, "opusc", true))
			{
				...
				switch(slot)
				{
					case 1: 
					{
						format(msg, sizeof(msg), "Opuściłeś/aś grupę %s.", GroupCache[PlayerCache[playerid][pGroup]][gName]);
						PlayerCache[playerid][pGroup] = 0;
					}
					case 2: 
					{
						format(msg, sizeof(msg), "Opuściłeś/aś grupę %s.", GroupCache[PlayerCache[playerid][pGroup2]][gName]);
						PlayerCache[playerid][pGroup2] = 0;
					}
					case 3: 
					{
						format(msg, sizeof(msg), "Opuściłeś/aś grupę %s.", GroupCache[PlayerCache[playerid][pGroup3]][gName]);
			...
			else if(!strcmp(sub, "zapros", true))
			{
				...
				switch(slot)
				{
					case 1:
					{
						...
						return SendClientMessage(playerid, COLOR_GRAY, "Nie posiadasz uprawnień do zapraszania.");
					}
					case 2:
					{
			...
			else if(!strcmp(sub, "v", true))
			{
				...
				return ShowDialogInfo(playerid, "Do tej grupy nie zostały przypisane jeszcze żadne pojazdy.");
			}
			else if(!strcmp(sub, "z", true))
			{
				...
				PlayerTextDrawHide(playerid, ObjectInfo[playerid]);
				if(!count)
				return ShowDialogInfo(playerid, "Nie znaleziono żadnych zgłoszeń");
...
CMD:agrupa (playerid, params[])
{
	...
	if(limit > 20 || limit < 1)
	return SendClientMessage(playerid, COLOR_GRAY, "Limit pojazdów grupowych musi mieścić się pomiędzy wartościami 1-20.");
...
CMD:mc (playerid, params[])
{
	...
	// Player Edit object
	new object_uid;
	mysql_query("SELECT Max(uid) FROM objects");
...
CMD:mpick (playerid, params[])
{
	...
	if(!objectid)
	return SendClientMessage(playerid, COLOR_GRAY, "Złap obiekt (najedź kamerą na obiekt, który chcesz edytować)");
...
cmd:sp (playerid, params[])
{
	...
	PenalityTimer = SetTimer("HidePenalityTextDraw", 15000, false);
	format(str, sizeof(str), "> Otrzymałeś SPoints (%d) od %s. Powód: %s", sp, ReturnPlayerName(playerid), reason);
...
CMD:pay (playerid, params[])
{
	...
	format(msg, sizeof(msg), "podaje trochę gotówki %s.", strreplace(ReturnPlayerName(targetid), '_', ' '));
	SendPlayerMe(playerid, msg);
...
CMD:skin (playerid, params[])
{
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Poprawne użycie /skin [ID/Część nazwy gracza] [skin ID]");
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Skin ID musi mieścić się w przedziale 0-311.");
	...
	if(targetid != playerid)
	{
		format(msg, sizeof(msg), "%s zmienił(a) Twój skin na skin ID: %d", ReturnPlayerName(playerid), skinid);
		SendClientMessage(targetid, COLOR_GRAY, msg);

		format(msg, sizeof(msg), "Zmieniłeś(aś) skin %s na skin ID: %d", ReturnPlayerName(targetid), skinid);
		SendClientMessage(playerid, COLOR_GRAY, msg);
	}
...
CMD:skuj (playerid, params[])
{
	...
	if(pCuffed[playerid])
	return SendClientMessage(playerid, COLOR_GRAY, "Nie możesz użyć kajdanek będąc skutym.");
	if(playerid == targetid)
	return SendClientMessage(playerid, COLOR_GRAY, "Nie możesz skuć samego siebie.");
	if(!pLogged[targetid])
	return SendClientMessage(playerid, COLOR_GRAY, "Gracz nie jest podłączony do serwera lub nie jest zalogowany.");
...
public CuffedTimer(playerid, targetid)
{
	if(!PlayerCache[playerid][pUID])
	{
		SetPlayerSpecialAction(targetid, SPECIAL_ACTION_NONE);
		pCuffed[targetid] = false;
		SendClientMessage(targetid, COLOR_GRAY, "Gracz który Ciebie skuł wyszedł z gry.");
		KillTimer(pCuffedTimer[targetid]);
	}
...
CMD:ap(playerid, params[])
{
	if(PlayerCache[playerid][pLevel] >= ADMINISTRATION)
	{
		...
		return SendClientMessage(playerid, COLOR_GRAY, "Poprawne użycie: /ap [Część/nazwa pojazdu] [ID koloru] [ID koloru 2] [wartość syreny]");
		new model;
		...
		if(!strcmp(sub, "namierz", true))
		{
			if(VehicleCache[vuid][vState] == 2)
			{
				...
				if(PlayerCache[playerid][pGroup] == guid)
				{
					if(!PlayerCache[playerid][pGroupVehicle])
					{
						return SendClientMessage(playerid, COLOR_GRAY, "Aby namierzać pojazdy grupowe musisz mieć odpowiednie uprawnienia nadane przez lidera.");
					}
				}
				if(PlayerCache[playerid][pGroup2] == guid)
				{
			...
			if(VehicleCache[vuid][vState] == 0 && VehicleCache[vuid][vOwner] != PlayerCache[playerid][pUID])
			{
				SendClientMessage(playerid, COLOR_GRAY, "Możesz namierzać tylko pojazdy należące do Ciebie.");
				return ClearAnimations(playerid);
			}
			if(VehicleCache[vuid][vID] == 0)
...
CMD:vdel (playerid, params[])
{
	if(PlayerCache[playerid][pLevel] < ADMINISTRATION)
	return 1;
	new vuid; if(sscanf(params, "i", vuid)) return SendClientMessage(playerid, COLOR_GRAY, "Poprawne użycie: /vdel [UID pojazdu] [-1 by zniszczyć wszystkie pojazdy z gry]");
	if(vuid == -1)
	{
		new count;
		for(new i; i<LastvUID; i++)
		...
		new msg[128];
		if(count)
		format(msg, sizeof(msg), "Pomyślnie usunięto %d pojazdów.", count);
		else
		return SendClientMessage(playerid, COLOR_GRAY, "Żaden poojazd nie został jeszcze utworzony, więc żaden nie mógł zostać usunięty.");
		SendClientMessage(playerid, COLOR_GRAY, msg);
		format(msg, sizeof(msg), "(( %d.%s %s usunął wszystkie pojazdy z mapy ))", playerid, GetRankName(PlayerCache[playerid][pLevel]), ReturnPlayerName(playerid)); 
		return SendMessageToAdmins(msg);
	}
	if(vuid < 0 || vuid >= MAX_VEHICLES)
	return SendClientMessage(playerid, COLOR_GRAY, "Wprowadzono niepoprawną wartość.");
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Ten pojazd został już usunięty z gry.");
...
CMD:idel (playerid, params[])
{
	if(PlayerCache[playerid][pLevel] < ADMINISTRATION)
	return 1;
	new iuid; if(sscanf(params, "i", iuid)) return SendClientMessage(playerid, COLOR_GRAY, "Poprawne użycie: /idel [UID przedmiotu] lub [-1] by pozbyc sie wszystkich przedmiotow.");
	if(iuid == -1)
	{
		new count;
		for(new i; i<MAX_ITEMS; i++)
		...
		new msg[128];
		if(count)
		format(msg, sizeof(msg), "Pomyślnie usunięto %d przedmiotów.", count);
		else
		return SendClientMessage(playerid, COLOR_GRAY, "Żaden przedmiot nie został jeszcze utworzony, więc żaden nie został usunięty.");
		SendClientMessage(playerid, COLOR_GRAY, msg);
		format(msg, sizeof(msg), "(( %d.%s %s usunął wszystkie przedmioty z mapy ))", playerid, GetRankName(PlayerCache[playerid][pLevel]), ReturnPlayerName(playerid)); 
		return SendMessageToAdmins(msg);
	}
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Ten przedmiot został już usunięty.");
...
{
	new Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz, pvw = GetPlayerVirtualWorld(playerid), query[256], data[1024],
...
CMD:rangasztangapizda (playerid, params[])
{
...
{
	if(!ObjectCache[playerid][oUID])
	return SendClientMessage(playerid, COLOR_GRAY, "Nie edytujesz teraz żadnego obiektu.");
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Owner bramy musi znajdować się w przedziale 0-2.");

	if(!ObjectCache[playerid][oGate])
	return SendClientMessage(playerid, COLOR_GRAY, "Ten obiekt nie jest bramą.");
	...
	if(ownertype == 0)
	{
		...
		mysql_query(query);
		return SendClientMessage(playerid, COLOR_GRAY, "Od teraz każdy może poruszać tą bramą.");
	}
	if(ownertype == 1)
	{
	...
	if(ownertype == 2)
	{
		...
		if(Isnull(list))
		return ShowDialogInfo(playerid, "Nie znaleziono żadnych grup.");
...
{
	if(!IsPlayerInAnyVehicle(playerid))
	return SendClientMessage(playerid, COLOR_GRAY, "Musisz znajdować się w pojeździe by użyć tej funkcji.");
	if(GetPlayerVehicleSeat(playerid) != 0)
	return SendClientMessage(playerid, COLOR_GRAY, "Musisz znajdować się na miejscu kierowcy by móc użyć tej funkcji.");
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Nie znajdujesz się na służbie odpowidniej grupy by móc użyć tej funkcji.");
}
...
/*stock SendSMSToPlayer(playerid, sms[])
{
			...
			if(ItemCache[i][iType] == 2)
			{
				if(ItemCache[i][iOwner] == PlayerCache[playerid][pUID])
				{
					if(ItemCache[i][iActive])
					{
						...
						return SendClientMessage(playerid, COLOR_GRAY, "Tip: /sms [numer] [treść]");
						if(num <= 50000)
						return SendClientMessage(playerid, COLOR_GRAY, "Wybrany numer jest nieprawidłowy.");
						for(new j=1; j<MAX_ITEMS; j++)
						{
							if(ItemCache[j][iType] == 2)
							{
								if(ItemCache[j][iVal]+50000 == num)
								{
									if(!ItemCache[j][iActive])
									{
										return SendClientMessage(playerid, COLOR_GRAY, "Osoba do której dzwonisz ma wyłączony telefon.");
									}
									for(new k; k<=GetPlayerPoolSize(); k++)
									{
										if(IsPlayerConnected(k))
										{
											if(pUID[k] == ItemCache[j][iOwner])
											{
												...
												format(msg, sizeof(msg), "* %s otrzymała wiadomość SMS *", RPName(playerid));
												else
												format(msg, sizeof(msg), "* %s otrzymał wiadomość SMS *", RPName(playerid));
												return Do(playerid, msg);
											}
										}
									}
									return SendClientMessage(playerid, COLOR_GRAY, "Abonament jest nieosiągalny.");
								}
							}
						}
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Nie posiadasz telefonu lub jest on wyłączony.");
}*/
...
CMD:d (playerid, params[])
{
	if(GroupCache[pDuty[playerid]][gType] == 1 || GroupCache[pDuty[playerid]][gType]  == 2 ||  GroupCache[pDuty[playerid]][gType]  == 16)
	{
		new text[128];
		if(sscanf(params, "s[128]", text))
		return SendClientMessage(playerid, COLOR_GRAY, "Tip: /d [treść]");
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Musisz być na służbie odpowiedniej grupy by użyć tej komendy.");
}

CMD:aplikuj (playerid, params[])
{
	if(GroupCache[pDuty[playerid]][gType] != 2)
	return SendClientMessage(playerid, COLOR_GRAY, "Nie znajdujesz się na służbie odpowiedniej grupy by móc użyć tej komendy.");
	new targetid;
	if(sscanf(params, "r", targetid))
	return SendClientMessage(playerid, COLOR_GRAY, "Tip: /aplikuj [ID/Część nazwy gracza]");
	if(!pLogged[targetid])
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Nie możesz zaaplikować leku gdy gracz ma status BW.");
...
/*CMD:bus (playerid, params[])
{
	...
	return ShowDialogInfo(playerid, "Nie znaleziono żadnych przystanków autobusowych znajdujących się w pobliżu.");
}*/
...
CMD:vw (playerid, params[])
{
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Tip: /vw [ID/Część nazwy gracza] [Virtual World]");
	if(!pLogged[targetid])
	return SendClientMessage(playerid, COLOR_GRAY, "Gracz wylogował się lub wyszedł z gry.");
...
CMD:gdel (playerid, params[])
{
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Użyj: /gdel [UID grupy] by wyjebać grupe w powietrze.");
	DestroyGroup(guid);
...
/*CMD:przetrzymaj (playerid, params[])
{
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Musisz znajdować się na służbie odpowiedniej grupy by móc użyć tej komendy.");
	for(new i; i<MAX_OBJECTS; i++)
	{
		if(ObjectCache[i][oState] == 0)
		{
			if(ObjectCache[i][oVW] == GetPlayerVirtualWorld(playerid))
			{
				if(IsPlayerInRangeOfPoint(playerid, 5.0, ObjectCache[i][oX], ObjectCache[i][oY], ObjectCache[i][oZ]))
				{
					...
					{
						new targetid, time; if(sscanf(params, "ri", targetid, time)) return SendClientMessage(playerid, COLOR_GRAY, "Tip: /przetrzymaj [ID/Część nazwy gracza] [czas przetrzymania w minutach]");
						if(!pLogged[targetid])
						return SendClientMessage(playerid, COLOR_GRAY, "Gracz wylogował się lub wyszedł z gry.");
						new Float:X, Float:Y, Float:Z; GetPlayerPos(targetid, X, Y, Z);
						if(GetPlayerVirtualWorld(playerid) != GetPlayerVirtualWorld(targetid) || !IsPlayerInRangeOfPoint(playerid, 5.0, X, Y, Z))
						return SendClientMessage(playerid, COLOR_GRAY, "Wybrany gracz jest zbyt daleko.");
						if(time < 0 || time > 120)
						return SendClientMessage(playerid, COLOR_GRAY, "Minimalny czas przetrzymania musi mieścić się pomiędzy 0-120 minut.");
						...
						new str[256]; format(str, sizeof(str), "Przetrzymano gracza %s na %d min.\nWróć tutaj by wypuścić gracza po jego odsiadce.", RPName(targetid), time);
						ShowDialogInfo(playerid, str);
						format(str, sizeof(str), "%s nadał Ci przetrzymanie na %d min.\nNie możesz opuścić budynku w którym jesteś przetrzymywany.", RPName(playerid), time);
...
CMD:blokuj (playerid, params[])
{
	...
	{
		if(IsPlayerInAnyVehicle(playerid))
		return SendClientMessage(playerid, COLOR_GRAY, "Wysiądź z pojazdu.");
		new vid = GetPlayerCameraTargetVehicle(playerid);
		if(vid != INVALID_VEHICLE_ID)
		{
			...
			if(IsPlayerInRangeOfPoint(playerid, 5.0, X, Y, Z))
			{
				...
				if(cost < 1 || cost > 3000)
				return SendClientMessage(playerid, COLOR_GRAY, "Koszt blokady musi znajdować się w przedziale $1-$3000.");
				new vuid = GetVehicleUID(vid);
				if(VehicleCache[vuid][vBanCost])
				return SendClientMessage(playerid, COLOR_GRAY, "Ten pojazd posiada już aktywną blokadę na koło.");
				VehicleCache[vuid][vBanCost] = cost;
				VehicleCache[vuid][vBanReason] = reason;
				format(reason, sizeof(reason), "nakłada blokadę na koło pojazdu %s", GetVehicleName(VehicleCache[vuid][vModel]));
		...
		{
			return SendClientMessage(playerid, COLOR_GRAY, "Musisz skierować swoją kamerę na pojazd i stanąć przy nim, by go zablokować!");
		}
	}
	return SendClientMessage(playerid, COLOR_GRAY, "Nie znajdujesz się na służbie odpowiedniej grupy by móc użyć tej komendy.");
}
	...
	if(GroupCache[pDuty[playerid]][gType] == 1 || GroupCache[pDuty[playerid]][gType] == 16)
	{
		...
		if(vid != INVALID_VEHICLE_ID)
		{
			...
			if(IsPlayerInRangeOfPoint(playerid, 5.0, X, Y, Z))
			{
				...
				return SendClientMessage(playerid, COLOR_GRAY, "Ten pojazd nie posiada blokady na koło.");
				GameTextForPlayer(playerid, "~w~blokada zdjeta", 3000, 4);
				VehicleCache[vuid][vBanCost] = 0;
				new reason[128]; reason = "";
				VehicleCache[vuid][vBanReason] = reason;
				format(reason, sizeof(reason), "zdejmuje blokadę na koło pojazdu %s", GetVehicleName(VehicleCache[vuid][vModel]));
		...
		{
			return SendClientMessage(playerid, COLOR_GRAY, "Musisz skierować swoją kamerę na pojazd i stanąć przy nim, by go odblokować!");
		}
	}
	return SendClientMessage(playerid, COLOR_GRAY, "Nie znajdujesz się na służbie odpowiedniej grupy by móc użyć tej komendy.");
}
...
stock GetMagazineStatus()
{
	...
	format(status, sizeof(status), ""HEX_RED"Nieokreślona");
	else
...
CMD:paczka (playerid, params[])
{
	...
	if(!IsPlayerInAnyVehicle(playerid))
	return ShowDialogInfo(playerid, "Musisz znajdować się w pojeździe by móc użyć wybrać paczkę do dostarczenia.");
	if(ServerSettings[magazinePosX] == 0 && ServerSettings[magazinePosY] == 0 && ServerSettings[magazinePosZ] == 0)
	return ShowDialogInfo(playerid, "Pozycja magazynu nie została jeszcze ustalona przez Administrację.\nPowiadom kogoś z ekipy widocznej pod /a!");
	...
	if(Isnull(list))
	return ShowDialogInfo(playerid, "Aktualnie nie ma żadnych paczek do dostarczenia.");
...
CMD:actortext (playerid, params[])
{
	...
	new actorid = GetPlayerCameraTargetDynActor(playerid);
	if(actorid == 0)
	return SendClientMessage(playerid, COLOR_GRAY, "Skieruj swą kamerę na aktora, którego chcesz namierzyć.");
	new text[256];
...
CMD:getneardoor (playerid, params[])
{
	for(new i; i<LastdUID; i++)
	{
		if(!DoorCache[i][dDestroyed])
		{
			...
			{
				new msg[128]; format(msg, sizeof(msg), "Drzwi UID: %d | Drzwi są przejściem: %s", i, DoorCache[i][dConnect] ? ("Tak") : ("Nie"));
				return SendClientMessage(playerid, COLOR_GRAY, msg);
			}
		}
	}
...
CMD:doorowner (playerid, params[])
{
	...
	return SendClientMessage(playerid, COLOR_GRAY, "Tip: /doorowner [UID drzwi] [ID/Część nazwy gracza]");
	if(duid < 1 || duid >= MAX_DOORS)
	return SendClientMessage(playerid, COLOR_GRAY, "Wprowadzono nieprawidłową wartość.");
	DoorCache[duid][dPlayerUID] = targetid;
	DoorCache[duid][dGroupUID] = 0;
	new msg[128]; format(msg, sizeof(msg), "Drzwi(%d) zostały podpisane. Nowy właściciel: %s", duid, RPName(targetid));
	return SendClientMessage(playerid, COLOR_GRAY, msg);
}
...
CMD:gvlimit (playerid, params[])
{
	...
	if(limit < 1 || limit > 20)
	return SendClientMessage(playerid, COLOR_GRAY, "Możesz ustalić limit tylko od 1 do 20 pojazdów.");
	if(guid < 1 || guid >= MAX_GROUPS)
	return SendClientMessage(playerid, COLOR_GRAY, "Nieprawidłowa wartość guid.");
	if(GroupCache[guid][gState] == 1)
	return SendClientMessage(playerid, COLOR_GRAY, "Ta grupa została usunięta.");
	GroupCache[guid][gVehicleLimit] = limit;
	new msg[128]; format(msg, sizeof(msg), "Limit pojazdów dla grupy %s (%d) został zmieniony na: %d pojazdów.", GroupCache[guid][gName], guid, limit);
	return SendClientMessage(playerid, COLOR_GRAY, msg);
}
...
CMD:cennik (playerid, params[])
{
	new duid = GetPlayerDoorUID(playerid);
	if(duid == 0 || DoorCache[duid][dGroupUID] == 0)
	return ShowDialogInfo(playerid, "Musisz znajdować się w biznesie grupy by skorzystać z tej funkcji.");
	...
	new targetid;
	if(sscanf(params, "r", targetid))
	return SendClientMessage(playerid, COLOR_GRAY, "Poprawne użycie: /cennik [ID/Część nazwy gracza]");
	if(!pLogged[targetid])
	return SendClientMessage(playerid, COLOR_GRAY, "Gracz nie jest podłączony do serwera lub wylogował się.");
	if(!IsPlayerInRangeOfPlayer(playerid, targetid, 5.0))
	return SendClientMessage(playerid, COLOR_GRAY, "Wybrany gracz jest zbyt daleko.");
	
	new count, list[1025]="#\tNazwa produktu\tCena\tIlość\n", info[128];
	for(new i; i<MAX_ITEMS; i++)
	{
		if(ItemCache[i][iState] == ITEM_STATE_PRODUCT)
		{
...
CMD:podaj (playerid, params[])
{
	new duid = GetPlayerDoorUID(playerid);
	if(duid == 0 || DoorCache[duid][dGroupUID] == 0)
	return ShowDialogInfo(playerid, "Nie znajdujesz się w wewnątrz budynku grupy.");
	...
	new targetid, amount;
	if(sscanf(params, "r", targetid))
	return SendClientMessage(playerid, COLOR_GRAY, "Tip: /podaj [ID/Część nazwy gracza] [ilość]");
...
CMD:live (playerid, params[])
{
	...
	if(duid == 0)
	{
				...
				if(IsPlayerInRangeOfVehicle(playerid, i, 50.0))
				{
					if(VehicleCache[vuid][vState] == VEHICLE_STATE_GROUP)
					{
						if(VehicleCache[vuid][vOwner] == guid)
						{
							new text[256];
							if(sscanf(params, "s[256]", text))
							return SendClientMessage(playerid, COLOR_GRAY, "Poprawne użycie: /live [wiadomość]");
		...
		return SendClientMessage(playerid, COLOR_GRAY, "By móc nadawać na zewnątrz, musisz być min. 50 m od grupowego News Vana lub News Mavericka.");
	}
	else
	{
		if(DoorCache[duid][dGroupUID] == guid)
...
CMD:ubranie (playerid, params[])
{
	...
	return ShowPlayerDialog(playerid, D_CLOTCHES, DIALOG_STYLE_LIST, "Wybierz opcję", "1\tUbranie\n2\tAkcesoria", "Wybierz", "Anuluj");
	return 1;
}
